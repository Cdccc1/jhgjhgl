# 这个是针对每个csv文件单独拟合（每个都拟合一份）且包含数据清洗（会剔除帧率饱和的）
# 但这个剔除会导致部分场景下有用的数据很少，可能还需要调整
...
TARGET_COL = "fps"  # 目标是帧率，我们拟合帧率的倒数
...
def filter_saturated_data(df: pd.DataFrame, fps_col="fps", fcpu_col="fcpu_ghz", fgpu_col="fgpu_ghz",
                          min_fps_change=0.4, window_size=5):
    """
    剔除帧率趋于饱和的数据点。
    按 fgpu 分组，在每组内检测 fps 是否局部变化过小。
    """
    df = df.copy()
    df = df.sort_values([fgpu_col, fcpu_col]).reset_index(drop=True)
    df["is_saturated"] = False

    for fgpu_val, group in df.groupby(fgpu_col):
        if len(group) < 3:
            continue
        fps_vals = group[fps_col].values
        indices = group.index.tolist()

        # 使用滑动窗口平滑后计算局部偏差
        win = min(window_size, len(fps_vals))
        fps_smooth = uniform_filter1d(fps_vals, size=win, mode='nearest')
        local_deviation = np.abs(fps_vals - fps_smooth)

        saturated_mask = local_deviation < min_fps_change
        df.loc[indices, "is_saturated"] = saturated_mask

    cleaned = df[~df["is_saturated"]].copy()
    return cleaned
...
def fit_one_file(df: pd.DataFrame):
    # 注意：需要保留 'fps' 用于清洗
    need = ["fcpu_ghz", "fgpu_ghz", "ddr_ghz", "frame_time", "fps"]
    work = df[need].copy().replace([np.inf, -np.inf], np.nan).dropna()
    work = work[(work["fcpu_ghz"] > 0) & (work["fgpu_ghz"] > 0) & (work["frame_time"] > 0)]

    if len(work) < 5:
        raise ValueError(f"Too few valid rows after initial cleaning: {len(work)}")

    # === 新增：剔除饱和数据 ===
    work_cleaned = filter_saturated_data(
        work,
        fps_col="fps",
        fcpu_col="fcpu_ghz",
        fgpu_col="fgpu_ghz",
        min_fps_change=0.4,  # 可根据数据调整，例如 0.3 ~ 0.6
        window_size=5
    )

    if len(work_cleaned) < 5:
        raise ValueError(f"Too few valid rows after saturation filtering: {len(work_cleaned)}")
