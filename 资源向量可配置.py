...
def load_and_preprocess(csv_path, temp_col=None):
    ...
    # 构建特征矩阵，列顺序固定，便于后续配置
    # 注意：后续配置中将通过列索引引用这些特征
    feature_columns = ['fcpu', 'fgpu', 'fddr_GHz', 'U_cpu', 'U_gpu', 'U_ddr', 'miss', temp_col]
    # 确保列存在
...
# 3. 多配置实验辅助函数
# ========================
def run_experiments(csv_path, configs, temp_col=None, verbose=True):
    """
    运行多个配置的拟合实验，返回每个配置的结果字典。
    configs: 列表，每个元素是配置字典（格式见ConfigurablePerformanceModel）
    """
    X, y, f_max_cpu, f_max_gpu, feature_names = load_and_preprocess(csv_path, temp_col)
    results = []
    for i, cfg in enumerate(configs):
        ...
        params, success = model.fit(X, y, verbose=verbose)
        rmse, mae, r2, y_pred = model.evaluate(params, X, y)
        # 收集参数
        ...
        if verbose:
            print(f"RMSE: {rmse:.4f} ms, MAE: {mae:.4f} ms, R²: {r2:.4f}")
    return results
...
if __name__ == "__main__":
    # 定义特征名称（与load_and_preprocess中一致）
    # 可用的特征: 'U_cpu', 'U_gpu', 'U_ddr', 'miss', 'temp'
    ...
    # 配置5: 空模型（只有截距，即常数α和τ0/τ1/λ，完全不依赖资源向量）
    config_constant = {
        'alpha_cpu': [],
        'alpha_gpu': [],
        'tau0':      [],
    ...
    # 可以添加更多配置，例如只去掉温度中的某一部分等
    configs_to_run = [config_full, config_no_temp, config_only_util, config_only_miss, config_constant]
    
    # 运行实验（请替换为实际CSV路径）
    results = run_experiments('your_data.csv', configs_to_run, temp_col='temp_soc')
    
    # 打印汇总比较表
    print("\n\n========== 汇总比较 ==========")
    for i, res in enumerate(results):
        print(f"配置{i+1}: RMSE={res['rmse']:.4f}, MAE={res['mae']:.4f}, R²={res['r2']:.4f}")
